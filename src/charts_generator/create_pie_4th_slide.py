...
from utils import SUNBURST_CHANNEL_COLORS, SENTIMENT_COLORS
...
def draw_connection_arrow(ax, direction='right', total=0, text_label='THẢO LUẬN', percentage=None, color='#000000'):
    if direction == 'right':
        ...
        ax.text(2.1, 0.1, total, ha='center', va='center', fontsize=11, color='black', fontweight='bold')
        ax.text(2.1, -0.12, text_label, ha='center', va='center', fontsize=4, color='black', fontweight='bold')
        if percentage is not None:
            ax.text(2.1, -0.4, f"{percentage}%", ha='center', va='center', fontsize=6, color=color, fontweight='bold')
            ax.text(2.1, -0.6, 'so với tuần trước', ha='center', va='center', fontsize=4, color='#DDDDDD')
    else:
        x = [0, -1.5, -1.5]
        y = [0, 0, -0.254]
        ax.annotate('', xy=(-2.7, -0.254), xytext=(-1.45, -0.254),
...
def render_single_chart(ax, df_sentiment, df_channel, legend_labels, colors_sentiment, colors_channel, colors,
    ...
    if total_last_week is not None:   
        percent_diff = np.round((total_value / total_last_week) * 100, 0) if total_last_week != 0 else 0
...
def generate_nested_chart(current_sentiment_data, current_channel_data, previous_sentiment_data, previous_channel_data):
    ...
    render_single_chart(
        ...
        legend_labels=current_channel_data.columns.tolist(),
        colors_sentiment=SENTIMENT_COLORS,
        colors_channel=SUNBURST_CHANNEL_COLORS,
        colors=SUNBURST_CHANNEL_COLORS,
        direction='right',
    ...
    render_single_chart(
        ...
        legend_labels=previous_sentiment_data.columns.tolist(),
        colors_sentiment=SENTIMENT_COLORS,
        colors_channel=SUNBURST_CHANNEL_COLORS,
